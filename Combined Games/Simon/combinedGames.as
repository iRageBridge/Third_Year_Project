...
randomGameGenorator(0);

function randomGameGenorator(randomGame: int):void{	
	var myColor:ColorTransform = this.transform.colorTransform;
	randomGame = Math.random()*2;
	if(randomGame == 0){
		...
		function gameOver():void{
			...
			randomGameGenorator(randomGame);
		}
		...
		victoryScreen.x = 2000;
		gameOverScreen.x = 2000;
		var plane: Plane = new Plane();
		var myChar: Char = new Char()
		var planeGameOverBool: Boolean = false;
		myChar.x = 567;
		myChar.y = 186;
		plane.x = 600;
		plane.y = 386;
		...
		function slide (ev:Event):void{
			...
			if(myChar.y == stage.stageHeight *0.8){
				planeGameOverBool = true;
			}
			if(planeGameOverBool == true){
				planeGameOver();
			}
		}	
